...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) { return <html lang="en" className="dark"><head><script dangerouslySetInnerHTML={{ __html: `try{var t=document.cookie.match(/(?:^|; )ni-theme=(dark|light)/)?.[1];if(t==='light'||(!t&&window.matchMedia('(prefers-color-scheme: light)').matches))document.documentElement.classList.remove('dark')}catch(e){}` }} /></head><body className={`${display.variable} ${mono.variable} font-sans`}>{children}</body></html> }
