      ...
      <div className="mt-16 grid gap-12 lg:grid-cols-[.75fr_1.25fr] lg:gap-20">
        <div role="tablist" aria-label="Products" className="divide-y divide-line border-y border-line">
          ...
          <div id={`product-preview-${active.slug}`} role="tabpanel" aria-live="polite" className="min-w-0">
            <div className="group relative isolate min-h-[26rem] overflow-hidden border border-line bg-background shadow-[0_24px_70px_rgba(0,0,0,.28)] sm:min-h-[34rem]" key={active.slug}>
              <div className="absolute inset-0 overflow-hidden bg-panel"><img src={preview} alt="" aria-hidden="true" className="absolute left-1/2 top-1/2 h-[125%] w-[125%] -translate-x-1/2 -translate-y-1/2 scale-110 object-contain opacity-20 blur-2xl grayscale transition-all duration-700" /></div>
              <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_20%,rgba(7,8,9,.35)_70%)]" />
              <div className="relative z-10 flex min-h-[26rem] items-center justify-center p-6 pb-32 sm:min-h-[34rem] sm:p-10 sm:pb-36"><div className="w-full overflow-hidden border border-line/80 bg-background/95 p-2 shadow-[0_25px_70px_rgba(0,0,0,.45)] transition-transform duration-700 group-hover:-translate-y-1 sm:p-3"><div className="flex items-center gap-2 border-b border-line pb-2"><span className="size-1.5 rounded-full bg-accent" /><span className="size-1.5 rounded-full bg-line" /><span className="size-1.5 rounded-full bg-line" /><span className="ml-2 truncate font-mono text-[9px] uppercase tracking-widest text-muted">naturalintellects / {active.slug}</span></div><div className="flex min-h-[15rem] items-center justify-center overflow-hidden bg-panel p-5 sm:min-h-[23rem] sm:p-8"><img src={preview} alt={`${active.name} product interface`} className="max-h-[20rem] max-w-full object-contain transition-transform duration-700 group-hover:scale-[1.02]" /></div></div></div>
              <div className="absolute inset-x-0 bottom-0 z-20 bg-gradient-to-t from-background via-background/95 to-transparent px-6 pb-6 pt-24 sm:px-10 sm:pb-8"><div className="eyebrow">{String(activeIndex + 1).padStart(2, '0')} · {active.status}</div><div className="mt-2 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><h3 className="text-3xl tracking-[-.04em] sm:text-4xl">{active.name}</h3><p className="mt-2 max-w-xl text-sm leading-relaxed text-muted">{active.solution}</p></div><Link href={`/products/${active.slug}`} className="group/link inline-flex shrink-0 items-center gap-2 font-mono text-[10px] uppercase tracking-widest text-foreground hover:text-accent">Explore product <ArrowUpRight className="size-4 transition-transform group-hover/link:-translate-y-1 group-hover/link:translate-x-1" /></Link></div></div>
            </div>
        </div>
      </div>
    </div>
